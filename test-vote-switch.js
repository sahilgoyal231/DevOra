const { Client, Databases, Users } = require("node-appwrite");
const client = new Client()
    .setEndpoint("https://sgp.cloud.appwrite.io/v1")
    .setProject("69f423940015d35d07aa")
    .setKey("standard_");

const databases = new Databases(client);
const users = new Users(client);

async function test() {
    try {
        const ulist = await users.list();
        const user = ulist.users[0];
        
        const qDoc = await databases.createDocument("main-DevOra", "questions", "unique()", {
            title: "Test Question",
            content: "Test Content",
            authorId: user.$id,
            tags: ["test"]
        });
        
        console.log("Upvote Response:", (await fetch("http://localhost:3000/api/vote", {
            method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ votedById: user.$id, voteStatus: "upvoted", type: "question", typeId: qDoc.$id })
        })).status);
        
        console.log("Downvote (Switch) Response:", (await fetch("http://localhost:3000/api/vote", {
            method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ votedById: user.$id, voteStatus: "downvoted", type: "question", typeId: qDoc.$id })
        })).status);
        
        console.log("Downvote (Revoke) Response:", (await fetch("http://localhost:3000/api/vote", {
            method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ votedById: user.$id, voteStatus: "downvoted", type: "question", typeId: qDoc.$id })
        })).status);
        
        await databases.deleteDocument("main-DevOra", "questions", qDoc.$id);
    } catch(e) {}
}
test();
