...
const Page = async ({
    ...
    const votes = await databases.listRows({ databaseId: db, tableId: voteCollection, queries: query });
    ...
    votes.rows = await Promise.all(
        votes.rows.map(async vote => {
            ...
            const answer = await databases.getRow({ databaseId: db, tableId: answerCollection, rowId: vote.typeId });
            const questionOfTypeAnswer = await databases.getRow({
                databaseId: db,
                tableId: questionCollection,
